...
class Admin extends REST_Controller {
    ...
    function forgot_pass_post() {
        if (checkparams($this->post(), array('vEmail'))) {
            //$postData = $this->post();
            //extract($postData);
            $vEmail = $this->post('vEmail');
            if ($this->admin_model->checkEmailAvailable($vEmail)) {
                //$new_password = random_string('alnum', 6);
                //$res = $this->admin_model->updateNewPassword($vEmail, $new_password);
                $data=array('Message' => 'Email has been sent on your email address to reset your password');
                //$this->send_mail($vEmail);
                $this->send_success($data);
            } else {
                $this->send_fail('No Account found for this email address');
            }
        } else {
    ...
    function send_mail($vEmail) {
        $query = $this->db->get_where('tbl_user', array('vEmail' => $vEmail));
        $row = $query->row();
        $name = $row->vUserName;
        $id = $row->iUserID;
        $email = $row->vEmail;

        $subject = 'Forgot Password STOL';
        ...
        if (checkparams($this->post(), array('vEmail', 'vPassword', 'ePlatform', 'vDeviceToken','dLat','dLong'))) {
            ...
            if ($this->admin_model->checkEmailAvailable($vEmail)) {
                ...
                if ($iUserID != '') {
                    ...
                    $profilepic = PROFILE_PIC_URL.$iUserID.'/thumb/'.$this->admin_model->getProfilePic($iUserID);
